...
describe("findAll", function () {
  ...
  test("works: by min employees + name", async function () {
    let companies = await Company.findAll(
        { minEmployees: 1, name: "1" });
    expect(companies).toEqual([
      {
        handle: "c1",
        name: "C1",
  ...
  test("works: by max employees + name", async function () {
    let companies = await Company.findAll(
        { maxEmployees: 1, name: "1" });
    expect(companies).toEqual([
      {
        handle: "c1",
        name: "C1",
  ...
  test("works: by min-max employees + name", async function () {
    let companies = await Company.findAll(
        { minEmployees: 1, maxEmployees: 1 , name: "1"});
    expect(companies).toEqual([
      {
        handle: "c1",
        name: "C1",
  ...
  test("works: empty list on nothing found", async function () {
    let companies = await Company.findAll({ name: "no results" });
    expect(companies).toEqual([]);
  });
